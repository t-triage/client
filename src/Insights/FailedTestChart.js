...
class FailedTestChart extends Component {
  state = {
    amountTests: [],
    selectedProduct: 0,
    productList: null,
      ...
      .then(res => {
        ...
        if (content[0]) {
          this.setState({
            ...
            axios.get(Api.getBaseUrl() + Api.ENDPOINTS.GetFailedTests + '?productId=' + this.state.selectedProduct )
                .then(res => {
                    this.setState({
  ...
  onProductChange(e) {
      ...
      axios.get(Api.getBaseUrl() + Api.ENDPOINTS.GetFailedTests + '?productId=' + value )
          .then(res => {
              this.setState({
                // TODO should shortlist on BE
                amountTests: res.data.slice(0, 6),
...
export default withStyles(styles)(FailedTestChart)
